    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="header-text" @click="playTitle">
              ...
              <div :class="{'left-to-right-animation':(currenClickNode==='djbf'||currenClickNode==='qwld') && ended && playTitleState}" :style="`left-to-right-animation:${time}s`" class="red-words">
                  <span>{{ serialNum }}</span>
                  .
                  <span v-for="(item, index) in title" :key="index"
                        @click="hzrdFun(item, `title`+index)"
          ...
          <div class="text">
            <div>
              <div class="paragraph" v-for="(item,index) in words" :key="index+'index'"  :class="{'isZoom':currenClickNode==='hzrd'}">
                <span v-for="(item1,index1) in item" :key="index1+'index1'"
                      @click="djbfFun(index, index1+1, item1)"
                       class="phrase"
                     >
                  <span
                      v-for="(item2,index2) in item1"
                      :key="index2+'index2'"
                      :class="{'blue':tagword.indexOf(item2)!==-1,'red':currenClickNode ==='hzrd' && 'index2'+index+''+index1+''+index2 === onlyKey&&ended }" @click="hzrdFun(item2,'index2' + index+'' + index1 + '' + index2)">
                    {{ item2 }}
                  </span>

                  <span
                    class="red-words"
                    :class="{'left-to-right-animation':(currenClickNode ==='djbf' || currenClickNode==='qwld') && currenLine === item1 && ended}"
                    :style="`animation-duration:${time}s`"
                  >
                    <span v-for="(item2, index2) in item1" :key="index2+'index2'">{{ item2 }}</span>
                  </span>
...
export default {
  ...
  methods:{
    ...
    menuPlay(fileName) {
      this.onlyKey  = ''
      this.currenLine = ''
      this.playFun(`/static/左侧按钮图标和声音/audio_${fileName}.mp3`)
    },
    // 汉字认读
    hzrdFun(word, key) {
        if (!(/[\u4e00-\u9fa5]/.test(word) && this.currenClickNode === 'hzrd')) return;
        this.onlyKey = key
        this.playFun(require(`../../static/xygq/${word}.mp3`))
    },
    // 全文朗读
    qwldFun() {
      setTimeout(() => {
    ...
    recursiveReading: debounce(function() {
      ...
      this.playFun2(`/static/xygq/${this.currenCount}line.mp3`)
      this.currenCount ++
    }),
    // 短句播放
    djbfFun(index,index1,currenLine) {
        ...
        sum += index1
        this.playFun(`/static/xygq/${sum}line.mp3`)
    },
    // 读标题
    playTitle() {
      if(this.currenClickNode === 'djbf'){
        this.playTitleState = true
        this.currenLine = ''
        this.playFun(`/static/xygq/0line.mp3`)
      }
    },
    playFun(url) {
      let self = this
